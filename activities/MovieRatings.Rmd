...
The observed data seems to be relatively similar to the expected data. There are some fluctuations, such as a 10 number difference between 1 star and 2 stars, which were expected to be the same.
...
chisq_result
```
df = degrees of freedom (# of categories - 1)
meets stat sig threshhold, not by much
...
Compare the chi-square statistic to the critical value, say whether you would accept or reject the null hypothesis and what that means.
    ...
    The chi-square number is larger than the critical value, though only by a little bit. This means that the null hypothesis should be rejected. The observed results are all within 10 to 15 of the expected results, though proportionally the shifts in the expected vs. observed are pretty significant. In a possible story about this, it's valid to write that there is a significant difference between observed and expected, though the chi-square doesn't really tell us why. That may be up to the reporter to keep digging to figure that out.
    
*chi-square doesn't tell us why, but can use it to test different scenarios
