...
ANSWER HERE
I think this is a useful model because it has statistical significance as a whole, and according to the multiple R-squared value shows that the model is a pretty good fit.
...
ANSWER HERE
I think points scored has the most impact on wins, which makes sense because that is what is typically used to calculate a win. The one that seems less impactful is takeaways. Yards gained isn't really correlated with wins, but it is correlated with points scored, which in turn contributes to wins.
...
This chart shows the relationship between different factors and the number of wins a team can have, so it could be a good predictor that can help you tell how well the team usually does. There are different factors that affect how often a team wins a game but these ones in particular have a level of correlation with the number of wins that would help predict the wins. The higher the adjusted R-squared number, the higher the accuracy of the correlation.
