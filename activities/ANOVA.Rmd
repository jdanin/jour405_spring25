...
library(tidyverse)
```
smaller SD = more clustered together, less spread out
```{r create-data}
...
The mean for community colleges is much lower than public and private, and while private is higher than public, the two are closer together than community is to either of them. The medians are relatively similar to the means per category, but standard deviation is where things shake up a bit. Community college has the largest standard deviation, and private has the smallest, showing that there is more variance at community colleges than at private schools. It's also interesting that the minimum score at a private school is almost 200 points higher than the average at a community college.
...
1. Is there a significant difference in SAT scores across college types?
    Yes, each p-value resulted in statistically significant scores across all three college types, meaning there's only a minuscule chance of observing the results if the null hypothesis is true.
2. Which specific college types differ from each other?
    Public and private are more similar to each other, but community differs greatly from the other two.
3. How large are these differences?
    The differences between community and the other two categories is much larger than the difference between public and private.
...
Data shows that students who attend private colleges and universities had higher SAT scores, while students who attend community colleges trail behind in scores.
