...
ANSWER HERE
The relationship is technically positive, but it is very very weak. What the linear regression line implies is that the higher the FARMS percentage, the higher the percentage of people who met or exceeded math expectations.
...
**close to 0 is an ideal median
the larger the residuals, the less confident you should be that this explains everything

ANSWER HERE
This model is not really worth using because it does not really explain all that much. It suggests that the relationship between FARMS percentage and math scores is somewhat related, but it doesn't provide enough evidence to support it.
...
ANSWER HERE
The residuals from the actual math scores are quite different from the predicted results. There are multiple schools where the residuals are over 30 percentage points away from the predicted result. The FARMS percentage is a proxy for income; it implies that the schools with higher FARMS percentage have families with lower incomes, and they are trying to see the connection between "income levels" (or just eating in general) and math scores. 
...
There seems to be some cherrypicking -- picking results that support your hypothesis while ignoring the rest/the ones that don't support your thesis. If there's any evidence at all that shows feeding kids helps them do better in school, the idea is that why shouldn't we be doing that? But the question is, does it work? The conclusions I've drawn from this model is that there is minimal evidence, but there could be enough to warrant investigating what other factors could affect math scores, and that could lead to a story. 
