...
ANSWER HERE
I don't think turnout is correlated to population because there are some cities that have nearly the same turnout percentage but vastly different population sizes. Turnout might be correlated to percentage of people with bachelor's degrees; I think towns with higher percentages of that tend to have a higher voter turnout. According to the scatterplots, the variable with the closest output to a linear relationship is percent unemployed. The lower the unemployment percentage, usually the lower the voter turnout.
...
ANSWER HERE
The model only deviates from the actual turnout by 0.02174, or 2%. The percentage of variability in turnout that is explained by the model is 0.8539 or 85%, but after accounting for the numner of variables, it explains 0.8023, or 80%. The F-statistic of 16.55 with a very low p-value (2.999e-06) suggests the model as a whole is statistically significant. Based on this information, the model seems to do a good job of predicting turnout, so it is a useful model. I don't believe any of the variables are insignificant, but the bachelor's degree percentage seems to be the most scattered and has the most outliers, so that could be a bit less significant (different than what I originally thought).
...
```{r}
# Model 2: without bachelor's
model2 <- lm(turnout ~ population + income + 
                   pct_unemployed + median_age + pct_white, data = md_turnout)
summary(model2)

# Model 3: without pct unemployed
model3 <- lm(turnout ~ population + income + pct_bachelors_higher 
                 + median_age + pct_white, data = md_turnout)
summary(model3)

# Model 4: without median_age
...
ANSWER HERE
The percentage of people who hold a bachelor's degree or higher is the least significant variable in this scenario; it is not one of the best indicators/predictors of voter turnout. Variables like median age and percentage of people who are unemployed are more important to account for when compiling data, but it is actually better to include the percentage of bachelor's degrees if including the other variables than not at all. Overall, I think these tests show that it is usually better to have more variables than less because we are aiming for the most detailed explanations of voter turnout possible.
