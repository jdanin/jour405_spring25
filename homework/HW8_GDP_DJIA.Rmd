...
```
* I know I did not add a title, I wasn't sure how to do it without messing up the entire chart.
...
From the correlation calculation alone, you can see that the correlation is high, with a score of 0.95. The scatterplot shows the correlation even stronger, and it shows a positive correlation with the line of best fit, which means that when one variable (like GDP) increases, so does the other variable (DJIA, in this case). The chart and the calculation suggest that these two variables, while one does not cause the other, they are closely correlated.
