...
ANSWER HERE
I think that some of the differences between expected and observed frequencies are meaningful because some of them are quite large. The observed frequencies for A and C grades are each 20 students apart from the expected, in different directions. The expected frequencies for D and F grades are the same, but their observed frequencies are 15 apart from each other.
...
ANSWER HERE
The chi-square value is greater than the critical value, so we do reject the null hypothesis.
...
ANSWER HERE
We rejected the null hypothesis, which was that the observed frequencies are consistent with the expected percentages, so it is reasonable to say that they are not consistent with one another. The notable discrepancies, as I pointed out above, include the observed frequencies for A and C grades each being 20 students apart from the expected, in different directions, and the expected frequencies for D and F grades being the same, but their observed frequencies are 15 apart from each other. The most notable discrepancy, more specifically, is that 20 more students than expected received A's, and 20 less than expected received C's, which is good. In fact, in general, people get higher grades than expected. I would maybe ask about some teaching techniques that the school employs that might explain the boost in grades or maybe about previous scores, because that might account for the lower expectation of good grades.
